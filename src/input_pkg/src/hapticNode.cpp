...
void HapticNode::commReadThread(int fd_client)
{
  ...
  while(1)
  {
    ...
    for(int i=0; i<NumOfDrivers; i++)
    {
      val[i] = 400*sin(counter*0.002);
    }
    counter++;

    // hapticMsg.array[0] = val[0];
    hapticMsg.array[1] = val[1];
    // hapticMsg.array[2] = val[2];
    // hapticMsg.array[3] = val[3];
    // hapticMsg.array[4] = val[4];
    // hapticMsg.array[5] = val[5];
